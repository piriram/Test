        ...
        try (Connection connection = DriverManager.getConnection(Config.URL, Config.USERNAME, Config.PASSWORD);
            ...
            switch (option) {
                case 1:
                    ...
                    text = "WHERE ID = " + id;
                    query = "SELECT * FROM "+Config.TABLE_NAME+" USE INDEX (idx_id) " + text;
                    countQuery = "SELECT COUNT(*) FROM "+Config.TABLE_NAME+" USE INDEX (idx_id) " + text;
                    break;
                case 2:
                    System.out.println("장소명을 입력하세요: ");
                    String 장소명 = scanner.next();
                    text = "WHERE 장소명 = '" + 장소명 + "'";
                    query = "SELECT * FROM "+Config.TABLE_NAME+" USE INDEX (idx_placeName) " + text;
                    countQuery = "SELECT COUNT(*) FROM "+Config.TABLE_NAME+" USE INDEX (idx_placeName)" + text;

                    break;
                case 3:
                    System.out.println("카테고리를 입력하세요: ");
                    String 카테고리 = scanner.next();
                    text = "FROM "+Config.TABLE_NAME+" WHERE 카테고리 = '" + 카테고리 + "'";
                    query = "SELECT * " + text;
                ...
                case 4:
                    ...
                    text = "FROM "+Config.TABLE_NAME+" WHERE 화장실_보유여부 = " + 화장실_보유여부;
                    query = "SELECT * " + text;
                    countQuery = "SELECT COUNT(*) FROM "+Config.TABLE_NAME+" WHERE 화장실_보유여부 = " + 화장실_보유여부;
                    break;
                case 5:
                    System.out.println("주차장 보유 여부를 입력하세요 (true/false): ");
                    boolean 주차장_보유여부 = scanner.nextBoolean();
                    text = "FROM "+Config.TABLE_NAME+" WHERE 주차장_보유여부 = " + 주차장_보유여부;
                    query = "SELECT * " + text;
                    countQuery = "SELECT COUNT(*) FROM "+Config.TABLE_NAME+" WHERE 주차장_보유여부 = " + 주차장_보유여부;
                    break;
                case 6:
                    System.out.println("개설연도를 입력하세요: ");
                    int 개설연도 = scanner.nextInt();
                    text = "FROM "+Config.TABLE_NAME+" WHERE 개설연도 = " + 개설연도;
                    query = "SELECT * " + text;
                    countQuery = "SELECT COUNT(*) FROM "+Config.TABLE_NAME+" WHERE 개설연도 = " + 개설연도;
                    break;
                case 7:
                    System.out.println("평점을 입력하세요: ");
                    double 평점 = scanner.nextDouble();
                    text = "FROM "+Config.TABLE_NAME+" WHERE 평점 = " + 평점;
                    query = "SELECT * " + text;
                    countQuery = "SELECT COUNT(*) FROM "+Config.TABLE_NAME+" WHERE 평점 = " + 평점;
                    break;
            }
        ...
        try (Connection connection = DriverManager.getConnection(Config.URL, Config.USERNAME, Config.PASSWORD);
            ...
            switch (option) {
                case 1:
                    ...
                    String query1 = "SELECT * FROM "+Config.TABLE_NAME+" WHERE 개설연도 >= " + startYear + " AND 개설연도 <= " + endYear;
                    String countQuery1 = "SELECT COUNT(*) FROM "+Config.TABLE_NAME+" WHERE 개설연도 >= " + startYear + " AND 개설연도 <= " + endYear;
                ...
                case 2:
                    ...
                    String query2 = "SELECT * FROM "+Config.TABLE_NAME+" WHERE 평점 >= " + minRating + " AND 평점 <= " + maxRating;
                    String countQuery2 = "SELECT COUNT(*) FROM "+Config.TABLE_NAME+" WHERE 평점 >= " + minRating + " AND 평점 <= " + maxRating;
